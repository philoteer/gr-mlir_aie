      ...
      _delay_history(_DELAY_SAMPLES, 0)
{
    ...
    _bo_in = xrt::bo(_device,
                     _IN_VECTOR_SIZE * sizeof(sync_long_input_type),
    ...
    _bo_out = xrt::bo(_device,
                      _VECTOR_SIZE * sizeof(sync_long_output_type),
                      XRT_BO_FLAGS_HOST_ONLY,
                      _kernel.group_id(3));
    ...
    _buf_in = _bo_in.map<sync_long_input_type*>();
    _buf_in_meta = _bo_in_meta.map<std::int32_t*>();
    _buf_out = _bo_out.map<sync_long_output_type*>();
    _buf_out_meta = _bo_out_meta.map<std::int32_t*>();

    std::memset(_buf_in, 0, _IN_VECTOR_SIZE * sizeof(sync_long_input_type));
    std::memset(
        _buf_in_meta, 0, _N_TILES * _METADATA_WORDS_PER_TILE * sizeof(std::int32_t));
    std::memset(_buf_out, 0, _VECTOR_SIZE * sizeof(sync_long_output_type));
    std::memset(
...
{
    const auto* in = static_cast<const sync_long_input_type*>(input_items[0]);
    auto* out = static_cast<sync_long_output_type*>(output_items[0]);
