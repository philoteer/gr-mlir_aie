      ...
      _delay_history(_DELAY_SAMPLES, 0)
{
    ...
    for (auto& slot : _slots) {
        ...
        slot.input = slot.input_bo.map<sync_long_input_type*>();
        slot.input_meta = slot.input_meta_bo.map<std::int32_t*>();
        slot.output = slot.output_bo.map<sync_long_output_type*>();
                                                   ...
                                                   gr_vector_void_star& output_items)
{
    ...
    const auto launch = [this, in, input_abs_start, tag_key](int chunk_idx) {
        ...
        for (int tile_idx = 0; tile_idx < _N_TILES; ++tile_idx) {
            ...
            std::copy(_delay_history.begin(),
                      _delay_history.end(),
                      slot.input + dst_start - _DELAY_SAMPLES);
    ...
    for (int i = 0; i < n_chunks; ++i) {
        ...
        for (int tile_idx = 0; tile_idx < _N_TILES; ++tile_idx) {
            const auto* tile_meta = slot.output_meta + tile_idx * _METADATA_WORDS_PER_TILE;
