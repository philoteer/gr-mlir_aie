/* -*- c++ -*- */
/*
 * Copyright 2026 gr-mlir_aie author.
 *
 * SPDX-License-Identifier: GPL-3.0-or-later
 */

#include "mlir_aie_cpp_bfloat16_impl.h"
#include <gnuradio/io_signature.h>

#include <algorithm>
#include <cstring>
#include <stdexcept>

/*
 * from header:
 * using gr_input_type = float;
using gr_output_type = float;

using aie_input_type = std::int16_t;
using aie_output_type = std::int16_t;
*/

namespace gr {
namespace mlir_aie {

mlir_aie_cpp_bfloat16::sptr mlir_aie_cpp_bfloat16::make(const char* path_xclbin,
                                                        const char* path_insts_bin,
                                                        const char* kernel_name,
                                                        int VECTOR_SIZE,
                                                        int num_slots)
{
    return gnuradio::make_block_sptr<mlir_aie_cpp_bfloat16_impl>(
        path_xclbin, path_insts_bin, kernel_name, VECTOR_SIZE, num_slots);
}


/*
 * The private constructor
 */
mlir_aie_cpp_bfloat16_impl::mlir_aie_cpp_bfloat16_impl(const char* path_xclbin,
                                                       const char* path_insts_bin,
                                                       const char* kernel_name,
                                                       int VECTOR_SIZE,
                                                       int num_slots)
    : gr::block("mlir_aie_cpp_bfloat16",
                gr::io_signature::make(
                    1 /* min inputs */, 1 /* max inputs */, sizeof(gr_input_type)),
                gr::io_signature::make(
                    1 /* min outputs */, 1 /*max outputs */, sizeof(gr_output_type)))
{
    if (num_slots < 1) {
        throw std::invalid_argument("num_slots must be at least 1");
    }

    _path_xclbin = path_xclbin;
    _path_insts_bin = path_insts_bin;
    _VECTOR_SIZE = VECTOR_SIZE;
    _kernel_name =  kernel_name; 
    set_output_multiple(_VECTOR_SIZE);
    _trace_size = 0;
    _opcode_run = 3;

    // Load instruction sequence
    _instr_v = test_utils::load_instr_binary(path_insts_bin);
    std::cout << "Sequence instr count: " << _instr_v.size() << "\n";
    
    // Start the XRT context and load the kernel
    test_utils::init_xrt_load_kernel(_device, _kernel, 1,
                                   path_xclbin,
                                   _kernel_name);

    std::cout << "kernel load ok";
    // set up the buffer objects
    _bo_instr = xrt::bo(_device, _instr_v.size() * sizeof(int),
                          XCL_BO_FLAGS_CACHEABLE, _kernel.group_id(1));
    std::cout << "Writing data into buffer objects.\n";

    // Copy instruction stream to xrt buffer object
    bufInstr = _bo_instr.map<void *>();
    memcpy(bufInstr, _instr_v.data(), _instr_v.size() * sizeof(int));
    _bo_instr.sync(XCL_BO_SYNC_BO_TO_DEVICE);

    _slots.resize(num_slots);
    for (auto& slot : _slots) {
        slot.input_bo = xrt::bo(_device,
                                _VECTOR_SIZE * sizeof(aie_input_type),
                                XRT_BO_FLAGS_HOST_ONLY,
                                _kernel.group_id(3));
        slot.output_bo = xrt::bo(_device,
                                 _VECTOR_SIZE * sizeof(aie_output_type) + _trace_size,
                                 XRT_BO_FLAGS_HOST_ONLY,
                                 _kernel.group_id(3));
        slot.input = slot.input_bo.map<aie_input_type*>();
        slot.output = slot.output_bo.map<aie_output_type*>();
        std::memset(slot.output,
                    42,
                    _VECTOR_SIZE * sizeof(aie_output_type) + _trace_size);
        slot.output_bo.sync(XCL_BO_SYNC_BO_TO_DEVICE);

        slot.run = xrt::run(_kernel);
        slot.run.set_arg(0, _opcode_run);
        slot.run.set_arg(1, _bo_instr);
        slot.run.set_arg(2, _instr_v.size());
        slot.run.set_arg(3, slot.input_bo);
        slot.run.set_arg(4, slot.output_bo);
    }
}

/*
 * Our virtual destructor.
 */
mlir_aie_cpp_bfloat16_impl::~mlir_aie_cpp_bfloat16_impl() {}

void mlir_aie_cpp_bfloat16_impl::forecast(int noutput_items,
                                          gr_vector_int& ninput_items_required)
{
    ninput_items_required[0] = std::max(_VECTOR_SIZE, noutput_items);
}


int mlir_aie_cpp_bfloat16_impl::general_work(int noutput_items,
                                             gr_vector_int& ninput_items,
                                             gr_vector_const_void_star& input_items,
                                             gr_vector_void_star& output_items)
{
    auto in = static_cast<const gr_input_type*>(input_items[0]);
    auto out = static_cast<gr_output_type*>(output_items[0]);


    const int n_chunks = std::min(ninput_items[0], noutput_items) / _VECTOR_SIZE;
    if (n_chunks == 0) {
        return 0;
    }

    const int depth = static_cast<int>(_slots.size());
    const auto launch = [this, in](int chunk_idx) {
        auto& slot = _slots[chunk_idx % _slots.size()];
        const gr_input_type* in_ptr = in + (chunk_idx * _VECTOR_SIZE);
        int j = 0;
        const int vec_step = 32;
        if (_VECTOR_SIZE >= vec_step) {
            for (; j <= _VECTOR_SIZE - vec_step; j += vec_step) {
                __m512 input_low = _mm512_loadu_ps(&in_ptr[j]);
                __m512 input_high = _mm512_loadu_ps(&in_ptr[j + 16]);
                __m512bh bf16_vec = _mm512_cvtne2ps_pbh(input_high, input_low);
                _mm512_storeu_si512(
                    static_cast<void*>(&slot.input[j]), _mm512_castbh_si512(bf16_vec));
            }
        }
        for (; j < _VECTOR_SIZE; j++) {
            slot.input[j] = float_to_bfloat16(in_ptr[j]);
        }

        slot.input_bo.sync(XCL_BO_SYNC_BO_TO_DEVICE);
        slot.run.start();
    };

    for (int i = 0; i < std::min(depth, n_chunks); ++i) {
        launch(i);
    }
    for (int i = 0; i < n_chunks; ++i) {
        auto& slot = _slots[i % depth];
        slot.run.wait();
        slot.output_bo.sync(XCL_BO_SYNC_BO_FROM_DEVICE);
        gr_output_type* out_ptr = out + (i * _VECTOR_SIZE);
        for (int j = 0; j < _VECTOR_SIZE; j++) {
            out_ptr[j] = bfloat16_to_float(slot.output[j]);
        }

        if (i + depth < n_chunks) {
            launch(i + depth);
        }
    }

    // ## Back to GNURadio
    int processed_items = n_chunks * _VECTOR_SIZE;
    consume_each(processed_items);
    
    return processed_items;
}

} /* namespace mlir_aie */
} /* namespace gr */
