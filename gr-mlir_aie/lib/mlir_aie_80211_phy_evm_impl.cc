...
mlir_aie_80211_phy_evm::sptr mlir_aie_80211_phy_evm::make(const char* path_xclbin,
...
mlir_aie_80211_phy_evm_impl::mlir_aie_80211_phy_evm_impl(const char* path_xclbin,
                                                 const char* path_insts_bin,
                                                 const char* kernel_name,
                                                  int VECTOR_SIZE,
                                                  double nominal_frequency,
    ...
    : gr::block("mlir_aie_80211_phy_evm",
                 gr::io_signature::make(1, 1, sizeof(phy_evm_input_type)),
                 gr::io_signature::make(1, 1, sizeof(phy_evm_output_type))),
      _N_TILES(N_TILES),
      _nominal_frequency(nominal_frequency)
{
    ...
    for (auto& slot : _slots) {
        slot.input_bo = xrt::bo(_device,
                                 _VECTOR_SIZE * sizeof(phy_evm_input_type),
        ...
        slot.output_bo = xrt::bo(_device,
                                  _VECTOR_SIZE * sizeof(phy_evm_output_type) + _trace_size,
                                  XRT_BO_FLAGS_HOST_ONLY,
        ...
        slot.input = slot.input_bo.map<phy_evm_input_type*>();
        slot.output = slot.output_bo.map<phy_evm_output_type*>();
...
void mlir_aie_80211_phy_evm_impl::set_nominal_frequency(double nominal_frequency)
{
    _nominal_frequency.store(nominal_frequency);
}

double mlir_aie_80211_phy_evm_impl::nominal_frequency() const
{
...
{
    auto in = static_cast<const phy_evm_input_type*>(input_items[0]);
    auto out = static_cast<phy_evm_output_type*>(output_items[0]);
    ...
    for (int i = 0; i < n_chunks; ++i) {
        ...
        for (int tile_idx = 0; tile_idx < _N_TILES; ++tile_idx) {
            ...
            std::memcpy(out + total_produced,
                        ...
                        tile_len * sizeof(phy_evm_output_type));
